...
it("session login 1 ", () =>{   // session will store and only work on one spec file 
    //cy.visit('/');
...
it.only("Amazon session login 1 ", () =>{   // session will store and only work on one spec file 
    cy.visit('/');
    cy.get("#twotabsearchtextbox").type("Mobile").type('{enter}');
})


it.only("Amazon session login 2", () =>{
    cy.visit('/');
    cy.get("#twotabsearchtextbox").type("Mobile").type('{enter}');
})
